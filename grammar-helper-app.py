...
DATA_PATH = Path("countability.json")
if DATA_PATH.exists():
    try:
...
tabs = st.tabs(
    ["Grammar · Countability Flashcards", "Other tabs (placeholder)"]
)
...
with tabs[0]:
    ...
    with left:
        ...
        with col_b:
            upload = st.file_uploader(
                "Load custom JSON", type=["json"], label_visibility="collapsed"
            )
            if upload:
                try:
                    ...
                    if isinstance(cards2, list) and cards2:
                        ...
                        st.session_state.idx = 0
                        st.session_state.revealed = False
                        st.success("Loaded custom deck.")
                    else:
                        st.error("Invalid JSON: missing 'countabilityCards' array.")
        ...
        if picked and st.session_state.deck:
            filtered = [c for c in CARDS if c.get("countability") in picked]
            if filtered:
    ...
    with mid:
        card = current_card()
        if not st.session_state.deck:
            st.info("No cards available. Check countability.json.")
        else:
            # Word face
            st.markdown(
                """
            ...
            if st.session_state.revealed:
                ...
                render_badge(typ or "unknown", color)
...
with tabs[1]:
    st.write("You can add more learning tools here later.")
